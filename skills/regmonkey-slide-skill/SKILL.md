...
description: "Author Quarto Reveal.js slide decks with the regmonkey-quarto-slides module (format: slides-revealjs). Use whenever the user wants to create, edit, or extend a slide in a project consuming this module — especially when they say 'スライド', 'qmd', 'deck', 'プレゼン', 'セクションを足して', '対比で見せて', 'AS-IS / TO-BE', or reference any house pattern (pentagon-box, square-box, hop-step-jump, info-box, summary, Index, Abstract)."
---


# regmonkey-slide-skill

スライドは Quarto `.qmd` を Reveal.js にコンパイルする．デザイン本体は regmonkey-quarto-slides モジュールの format extension（`_extensions/regmonkey/slides/revealjs.scss` + `scss/_*.scss` partials）と `_extensions/` のカスタムショートコード群に存在する．生 HTML や独自クラスを発明すると壊れる．この Skill は規約を体系化し，新しいスライドが既存 deck と同じ見た目になるよう保証する．
...
- **テンプレート群** — 即コピペ可能．**`templates/`（skill 配下）とモジュール直下の `template/`（分析プロジェクトでは通常 `_slides/template/`）はミラーではなく相補的**．下の表で場所を確認．書き直さずコピーして編集する．
...
7. **コードに `font-family` を設定しない．** プロジェクトは敢えて汎用 `monospace` を使う方針．上書きすると deck 間の見た目の一貫性が崩れる．
...
deck の配置は `notebook/<slug>/index.qmd`（per-deck ディレクトリ）または `notebook/<name>.qmd`（flat）のどちらでもよい（regmonkey-presentation repo では `posts/YYYY-MM-DD-<slug>/index.qmd`）．format 設定の本体（revealjs，1600×900，Meiryo，MathJax 3 等）は format extension `slides-revealjs` が供給し，ディレクトリの `_metadata.yml` が `format: slides-revealjs` を宣言するので，deck 内で重複指定しない．
...
| 4-5 | 並列観点 | horizontal-keypoints（N列） | `three-horizontal-points.qmd` を列追加で拡張（`references/components.md` 参照） |
| 3-6 | 連続プロセス（日次サイクル等） | step-arrow（active-phase-sm） | `references/components.md` の `.step-container` セクション参照 |
| 5+ | 並列分類（カテゴリ整理） | yaml2table 3-4 列 | `summary-yaml2table.qmd` |
...
**Summary — message-card スタック（デフォルト）：** deck 冒頭のテイクアウェイ提示やセクション recap は，`.message-card`（`.message-card-title-no-margin .center` の見出しバー + `.message-card-body .squaredmark` の本文）を 3〜5 枚，`{{< reveal_vspace 0.6em >}}` 区切りで縦に積む．テンプレートは `<module>/template/message-card.qmd`，完全な解説は `references/patterns.md` §3．

**Summary — yaml2table 3 列：** 各セクションが「**原則 + 具体アクション**」のセットで束ねられるとき，message-card スタックの代わりに `templates/summary-yaml2table.qmd` を使う．category（短いラベル）/ rule（1 文の主原則）/ actions（プロセスとして読める動詞句リスト）の 3 列構造．推奨ルールと完全な解説は `references/patterns.md` §3．

## regmonkey_index（章立て・目次スライド）の主要オプション

`.regmonkey_index` の YAML は format extension 同梱の `regmonkey_index_summary.js` が render 時に処理する．以下のキーは知らないと毎回ユーザーに教えてもらう羽目になるので最低限覚える．
...
場所の表記：`<module>/template/` は regmonkey-quarto-slides モジュール直下の `template/`（分析プロジェクトでは通常 `_slides/template/`），`templates/` はこの skill 内の `templates/`．

| ユーザーの依頼 | テンプレート | 場所 |
|---|---|---|
| 対比 / 課題と対応 | `pentagon-squared-box.qmd` | `<module>/template/` |
| 三段階の成長 / フェーズ展開（昇順） | `hop-step-jump.qmd` | `<module>/template/` |
| 三つの観点を横並び | `three-horizontal-points.qmd` | `<module>/template/` |
| 概念の定義スライド | `concept-explanation.qmd` | `<module>/template/` |
| 分析サマリースライド (4要素) | `analysis-summary.qmd` | `<module>/template/` |
| 章扉・セクション開始 | `meta-data-slide.qmd` | `<module>/template/` |
| 書籍紹介 | `book-template.qmd` | `<module>/template/` |
| 同じ枠の左右対比（情報密度高め） | `info-box-with-two-cols.qmd` | `<module>/template/` |
| ラベル＋タイトル＋本文のメッセージカード | `message-card.qmd` | `<module>/template/` |
...
ガイドライン系のカード例（regmonkey-presentation repo の `posts/2026-05-25-git-development-flow/index.qmd` が実例）：
```qmd
...
- **YAML の filter を忘れる．** `regmonkey_slide_editing` がないと `{{< fas >}}` のショートコードがリテラルテキストのまま表示される．
...
3. CSS 拡張は regmonkey-quarto-slides の `_extensions/regmonkey/slides/scss/_*.scss` partials を編集（component 系は `_components.scss`・layout 系は `_layouts.scss`・utility 系は `_utilities.scss`）．既存命名規則（`-400`/`-500` のサイズサフィックス，`-left` の位置サフィックス）に従い，対応するフェンス div を `<module>/template/` 配下の新ファイルに定義して再利用可能にする．変更はモジュール repo にコミットし，消費側プロジェクトは submodule bump で取り込む．
